...
Automated daily 9:00 AM (Asia/Singapore) summary of today's and tomorrow's events from the shared Google Calendar (**SW Leave & Events**) delivered directly to the team WhatsApp group (**CFMS / SW Team**).
...
```mermaid
flowchart LR
    A["Pipedream Cron Trigger<br/>Daily 09:00 AM SGT (01:00 UTC)"] --> B["Google Calendar Action<br/>Fetch SW Leave & Events"]
    ...
    E --> F["WhatsApp Group<br/>CFMS / SW Team"]
```
...
3. **Idempotency & Destination Locking:**
   - Messages are destination-locked to the exact pinned group JID (`120363023046007972@g.us`).
